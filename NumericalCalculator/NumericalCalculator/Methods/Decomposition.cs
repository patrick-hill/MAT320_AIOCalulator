...
namespace NumericalCalculator.Methods
{
    class Decomposition : BaseMethod
    {
        ...
        {
            // convert our matrix into this gay one...
             ...
             // allocates/creates a matrix initialized to all 0.0. assume rows and cols > 0
              ...
              for (int i = 0; i < size; ++i)
                  for (int j = 0; j < size; ++j)
                      m[i][j] = 0.0; // explicit initialization needed in some languages
      ...
      addToLog("The (combined) LUP decomposition of m is\n" + MatrixAsString(luMatrix));
      addToLog("The decomposition permutation array is: " + VectorAsString(perm));
      addToLog("The decomposition toggle value is: " + toggle);
      addToLog("\nThe lower part of LU is \n" + MatrixAsString(lower));
      addToLog("The upper part of LU is \n" + MatrixAsString(upper));

      double[][] inverse = MatrixInverse(m);
      addToLog("Inverse of m computed from its decomposition is\n" + MatrixAsString(inverse));

      double det = MatrixDeterminant(m);
      addToLog("Determinant of m computed via decomposition = " + det.ToString("F1"));

      addToLog("\nLinear system mx = b problem: \n");
      addToLog("3x0 + 7x1 + 2x2 + 5x3 = 49");
      addToLog(" x0 + 8x1 + 4x2 + 2x3 = 30");
      addToLog("2x0 +  x1 + 9x2 + 3x3 = 43");
      addToLog("5x0 + 4x1 + 7x2 +  x3 = 52");
      addToLog("");

      double[] b = new double[] { 49.0, 30.0, 43.0, 52.0 };
      addToLog("Solving system via decomposition");
      ...
      if (MatrixAreEqual(orig, m, 0.000000001) == true)
        addToLog("Product of L and U successfully unpermuted using perm array");
      ...
      addToLog("End matrix decomposition demo\n");
      Console.ReadLine();
    }
